export default async function handler(req, res) {
  ...
  if (!id) return res.status(400).json({ error: "Manca l'id della partita." });
  ...
  try {
    ...
    if (!fx.ok) return res.status(fx.status).json({ error: fx.data?.message || "Errore nel dettaglio match." });
