...
const CATEGORY_TREE = [
  ...
  { key:'nazionale', label:'Nazionale', items:[
    ...
    { name:'Qualificazioni Mondiali', code:'qualificazioni-mondiali', live:true },
    { name:'Qualificazioni Europei', code:'qualificazioni-europei', live:false, disabled:true },
    { name:'UEFA Nations League', code:'nations-league', live:true },
    { name:'Europei', code:'europei', live:false, disabled:true },
    { name:'Mondiali', code:'mondiali', live:true }
  ]}
];
...
const GENERIC_TRANSLATIONS = {
  ...
  'Goalkeeper':'Portiere','Defender':'Difensore','Midfielder':'Centrocampista','Attacker':'Attaccante','Forward':'Attaccante',
  'Left Winger':'Ala sinistra','Right Winger':'Ala destra','Centre-Forward':'Centravanti','Second Striker':'Seconda punta',
  'Centre-Back':'Difensore centrale','Left-Back':'Terzino sinistro','Right-Back':'Terzino destro','Defensive Midfielder':'Mediano',
  'Central Midfielder':'Centrocampista centrale','Attacking Midfielder':'Trequartista','Left Midfielder':'Centrocampista sinistro','Right Midfielder':'Centrocampista destro'
};
...
function topbar(){
  return `<div class="topbar"><div class="brand-inline" role="button" tabindex="0" onclick="setPage('home')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();setPage('home');}"><div class="logo" aria-hidden="true"><img src="logo.svg" alt="Logo Calcio Italiano" /></div><div><h1>Calcio Italiano</h1><small>Risultati, classifiche e approfondimenti sul calcio italiano</small></div></div><div class="topnav">${NAV_ITEMS.map(item=>`<button class="${state.page===item.key?'active':''}" onclick="setPage('${item.key}')"><span class="nav-ico">${item.icon}</span><span>${item.label}</span></button>`).join('')}</div></div>`;
}
...
function matchesList(){
  ...
  return `<div class="card section"><div class="rowflex" style="margin-bottom:10px"><h2>${currentMode()==='team' ? 'Partite dell\'Italia' : 'Partite'}</h2><div class="muted small">${esc(note)}</div></div>${toolbar()}${state.loading?`<div class="notice">Caricamento partite...</div>`:state.error?`<div class="notice">${esc(state.error)}</div>`:body}</div>`;
}

function standingTags(rank){
  const code = currentCode();
  const tags = [];
  if(code==='serie-a'){
    if(rank>=1 && rank<=4) tags.push('Champions League');
    else if(rank===5) tags.push('Europa League');
    else if(rank===6) tags.push('Conference League');
  }
  if(rank>=18) tags.push('Retrocessione');
  return tags.map(tag=>`<span class="standing-tag">${tag}</span>`).join(' ');
}
function standingsCard(){
  return `<div class="card section"><div class="rowflex" style="margin-bottom:10px"><h2>Classifica</h2><div class="muted small">${esc(state.competition)}</div></div>${state.standingsLoading?`<div class="notice">Caricamento classifica...</div>`:!state.standings.length?`<div class="notice">Classifica non disponibile per questa categoria.</div>`:`<table><thead><tr><th>#</th><th>Squadra</th><th>PT</th><th>G</th><th>V</th><th>N</th><th>P</th><th>DR</th></tr></thead><tbody>${state.standings.map(r=>`<tr class="${standingRowClass(r.rank)}"><td>${r.rank}</td><td>${teamLink(r.team.name,r.team.id)} ${standingTags(r.rank)}</td><td>${r.points}</td><td>${r.all.played}</td><td>${r.all.win}</td><td>${r.all.draw}</td><td>${r.all.lose}</td><td>${r.goalsDiff}</td></tr>`).join('')}</tbody></table>`}</div>`;
}
...
function syncHistory(mode='replace'){
  ...
  if(mode==='push') window.history.pushState(payload, '', window.location.pathname + window.location.search + window.location.hash);
  else window.history.replaceState(payload, '', window.location.pathname + window.location.search + window.location.hash);
}
function render(){ document.getElementById('app').innerHTML = `${topbar()}<div class="app">${sidebar()}${mainView()}</div>`; syncHistory('replace'); }
...
function setPage(page){
  if(page==='home'){ resetToHome(); render(); syncHistory('push'); return; }
  state.page=page; state.selected=null; state.selectedTeam=null; state.teamDirectorySearch=''; render(); syncHistory('push');
...
async function openNationalTeam(teamId, teamName){
  state.selected=null; state.selectedTeam={id:teamId,name:teamName}; state.teamLoading=true; state.teamData=null; state.teamTab='overview'; render(); syncHistory('push');
...
async function loadFixturesOnly(){
  if(currentMode()==='team'){
    ...
    const res = await fetch(`/api/national-fixtures?team=${teamId}`);
...
async function loadStandings(){
  if(currentMode()==='team'){ state.standings=[]; state.standingsLoading=false; render(); return; }
...
async function openMatch(id){
  state.selectedTeam=null; state.teamData=null; state.selected={loading:true}; render(); syncHistory('push');
...
function closeMatch(){ state.selected=null; render(); syncHistory('push'); }
...
function closeTeam(){ state.selectedTeam=null; state.teamData=null; state.teamLoading=false; state.teamTab='overview'; render(); syncHistory('push'); }
...
window.addEventListener('mousedown', (event) => {
  if(event.button===3){ event.preventDefault(); window.history.back(); }
  if(event.button===4){ event.preventDefault(); window.history.forward(); }
});
render();
