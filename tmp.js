...
const state = {
  page:'home', competition:'Serie A', filter:'all', search:'', fixtures:[], loading:false, error:'',
  ...
  transfers:[], transfersLoading:false, transfersNote:'', teamDirectorySearch:'', teamTab:'overview'
};
...
const STAT_LABELS = {
  ...
  'Big Chances Created':'Grandi occasioni create','Big Chances Missed':'Grandi occasioni sprecate'
};
const EVENT_DETAIL_LABELS = {
  ...
  'Red Card':'Espulsione','Substitution':'Sostituzione','Var':'VAR','Goal Disallowed':'Gol annullato'
};
...
function homeHero(){
  return `<div class="hero"><div class="card hero-main"><div class="muted small">${esc(currentMacro())} • ${esc(state.competition)}</div><h1>Il calcio italiano, in una vista più pulita.</h1><p>Ora hai una top bar separata, macro-categorie con sottocategorie, sezione squadre dedicata, mercato diviso in ufficiali e rumors e classifiche individuali più ordinate.</p><div class="pills"><span class="pill">Live</span><span class="pill">Classifica</span><span class="pill">Mercato</span><span class="pill">Top Marcatori</span><span class="pill">Squadre</span></div></div><div class="card hero-side"><h2>Stato rapido</h2><div class="kpi"><div class="row"><strong>Competizione attiva</strong><div class="muted small">${esc(state.competition)}</div></div><div class="row"><strong>Partite caricate</strong><div class="muted small">${state.fixtures.length || '—'}</div></div><div class="row"><strong>Ordine</strong><div class="muted small">Più recenti in alto</div></div></div></div></div>`;
}
...
function homeView(){ return `<div class="content">${homeHero()}<div class="grid grid2">${matchesList()}${standingsCard()}</div></div>`; }
...
function detailView(){
  ...
  return `<div class="content"><a class="back" onclick="closeMatch()">← Torna alla competizione</a><div class="grid grid2"><div><div class="card section"><div class="rowflex"><div><div class="muted small">${esc(m.league.name)} • ${localDate(m.fixture.date)}</div><div class="title">${teamLink(m.teams.home.name,m.teams.home.id)} vs ${teamLink(m.teams.away.name,m.teams.away.id)}</div><div class="muted small">${esc(m.fixture.venue?.name||'Stadio non disponibile')}</div></div><div class="badges"><span class="badge ${statusClass(m.fixture.status.short)}">${statusLabel(m)}</span><span class="scorebadge">${m.goals.home ?? '-'} - ${m.goals.away ?? '-'}</span></div></div></div><div class="card section"><h2>Eventi</h2>${events.length?events.map(e=>`<div class="event"><div class="rowflex"><strong>${e.time.elapsed?e.time.elapsed+"'":'-'} ${iconForEvent(e.type,e.detail)}</strong><span class="muted small">${esc(e.team?.name||'')}</span></div><div style="margin-top:6px">${esc(e.player?.name||'')}${e.assist?.name?' • 👟 '+esc(e.assist.name):''}</div><div class="muted small">${EVENT_DETAIL_LABELS[e.detail] || STAT_LABELS[e.type] || esc(e.type||'')}${e.detail && !EVENT_DETAIL_LABELS[e.detail] ? ' • '+esc(e.detail):''}</div></div>`).join(''):`<div class="muted small">Nessun evento disponibile.</div>`}</div>${statsHtml}</div><div>${lineupsHtml}</div></div></div>`;
}
...
function teamTabNews(d){
  ...
  return `<div class="card section tab-shell"><h2>Notizie / movimenti squadra</h2>${news.length?`<div class="list">${news.map(n=>`<div class="news-card"><strong>${esc(n.player?.name || 'Giocatore')}</strong><div class="directory-meta">${esc(n.teams?.out?.name || '—')} → ${esc(n.teams?.in?.name || '—')}</div><div class="directory-meta">${esc(n.update?.type || 'Aggiornamento')} • ${n.update?.date ? localDate(n.update.date) : 'n.d.'}</div></div>`).join('')}</div>`:`<div class="notice">Al momento qui compaiono movimenti ufficiali collegati alla squadra. Per vere news live serve integrare una fonte editoriale esterna.</div>`}</div>`;
}
function teamView(){
