...
const CATEGORY_TREE = [
  ...
  { key:'italiane-europa', label:'Italiane in Europa', items:[
    ...
    { name:'Supercoppa UEFA', code:'supercoppa-uefa', live:false, disabled:true }
  ]},
  { key:'nazionale', label:'Nazionale', items:[
    { name:'Italia', code:'italia', live:true, mode:'team', teamId:76 },
    { name:'Italia U21', code:'italia-u21', live:false, disabled:true },
    { name:'Qualificazioni Mondiali', code:'qualificazioni-mondiali', live:false, disabled:true },
    { name:'Qualificazioni Europei', code:'qualificazioni-europei', live:false, disabled:true },
    { name:'UEFA Nations League', code:'nations-league', live:false, disabled:true },
    { name:'Europei', code:'europei', live:false, disabled:true },
    { name:'Mondiali', code:'mondiali', live:false, disabled:true }
  ]}
];
...
function matchesList(){
  ...
  const body = !list.length
    ? `<div class="notice">Nessuna partita trovata per i filtri selezionati.</div>`
    : (state.filter==='finished' || state.filter==='live') 
      ? `<div class="list">${list.map(m=>`<div class="card match" onclick="openMatch(${m.fixture.id})"><div class="rowflex"><div><div class="muted small"><strong>${localDateLong(m.fixture.date)}</strong> • ${esc(m.league.name)} • ${localDate(m.fixture.date)}</div><div class="title">${esc(m.teams.home.name)} vs ${esc(m.teams.away.name)}</div></div><div class="badges"><span class="badge ${statusClass(m.fixture.status.short)}">${statusLabel(m)}</span><span class="scorebadge">${m.goals.home ?? '-'} - ${m.goals.away ?? '-'}</span></div></div></div>`).join('')}</div>`
      : renderMatchesGrouped(list);

  return `<div class="card section"><div class="rowflex" style="margin-bottom:10px"><h2>Partite</h2><div class="muted small">${esc(note)}</div></div>${toolbar()}${state.loading?`<div class="notice">Caricamento partite...</div>`:state.error?`<div class="notice">${esc(state.error)}</div>`:body}</div>`;
}

function standingsCard(){
  const labelForRow = rank => rank<=4 ? '<span class="standing-tag">Europa</span>' : rank>=18 ? '<span class="standing-tag">Zona calda</span>' : '';
...
function supportView(){
  return `<div class="content"><div class="support-wrap"><div class="support-story"><div class="card hero-main support-long"><div class="muted small">Supporta il progetto</div><h1>Calcio Italiano Live cresce grazie alla community</h1><p>Calcio Italiano Live nasce con un obiettivo chiaro: costruire una piattaforma moderna, veloce e piacevole da consultare, dedicata interamente al calcio italiano. Non solo Serie A, ma anche Serie B, Serie C, Serie D, Primavera, calcio femminile, coppe nazionali e competizioni europee con le squadre italiane protagoniste.</p><p>Il progetto unisce in un'unica web app partite live, calendario mensile, classifiche, schede squadra, statistiche tradotte in italiano, top giocatori, mercato e aggiornamenti continui. L'idea è offrire un'esperienza pulita e affidabile, pensata per chi vuole seguire il calcio italiano senza dispersione, con un'interfaccia ordinata e sempre più curata nel dettaglio.</p><p>Ogni donazione aiuta davvero a migliorare il progetto: copre i costi delle API, sostiene l'infrastruttura serverless, rende possibile testare nuove funzioni e permette di rifinire la parte grafica e l'esperienza utente. In pratica, il tuo supporto si trasforma in un sito più veloce, più completo e più professionale.</p><p>Calcio Italiano Live è un progetto indipendente, costruito con passione e con la volontà di crescere passo dopo passo. Sostenendolo, contribuisci a far nascere un punto di riferimento sempre più solido per chi ama seguire il calcio italiano ogni giorno.</p><div class="tricolor-band"><div class="green">Copertura ampia</div><div class="white">Esperienza pulita</div><div class="red">Sviluppo continuo</div></div><div class="hero-cta"><a class="paypal-btn" href="https://paypal.me/marcoteribia" target="_blank" rel="noopener noreferrer">Dona con PayPal</a><button class="ghost-btn" onclick="setPage('home')">Torna alla Home</button></div></div><div class="card section"><h2>Perché vale la pena supportarlo</h2><div class="support-grid"><div class="support-box"><strong>Prestazioni e stabilità</strong><div class="directory-meta">Le donazioni aiutano a sostenere API, hosting e chiamate serverless, mantenendo il sito reattivo e affidabile.</div></div><div class="support-box"><strong>Nuove funzionalità</strong><div class="directory-meta">Più supporto significa più tempo da investire in notifiche live, schede avanzate, filtri migliori e nuove sezioni.</div></div><div class="support-box"><strong>Qualità visiva</strong><div class="directory-meta">Ogni contributo aiuta a migliorare design, leggibilità e coerenza grafica, rendendo il sito sempre più premium.</div></div><div class="support-box"><strong>Progetto indipendente</strong><div class="directory-meta">Non è una piattaforma enorme: è un progetto reale, costruito con cura, passione e voglia di farlo crescere bene.</div></div></div></div></div><div class="support-visual"><div class="support-banner"><div class="select-note">Grazie davvero</div><h2 style="font-size:1.7rem;margin:8px 0 10px">Il tuo supporto non finanzia solo il presente: costruisce il futuro del progetto.</h2><p>Ogni contributo aiuta a rendere Calcio Italiano Live sempre più completo, ordinato e bello da usare, giorno dopo giorno.</p></div><div class="card hero-side"><h2>Profilo progetto</h2><div class="kpi"><div class="row"><strong>Autore</strong><div class="muted small">Gianmarco Teribia</div></div><div class="row"><strong>Identità</strong><div class="muted small">Progetto indipendente dedicato al calcio italiano</div></div><div class="row"><strong>Missione</strong><div class="muted small">Creare una web app credibile, completa e sempre più professionale.</div></div></div></div><div class="card hero-side"><h2>Cosa rende speciale il progetto</h2><div class="hero-list"><div class="feature"><span>Copertura</span><strong>dalle big alle categorie minori</strong></div><div class="feature"><span>Focus</span><strong>solo calcio italiano</strong></div><div class="feature"><span>Obiettivo</span><strong>diventare un punto di riferimento</strong></div></div></div></div></div></div>`;
}
...
function topView(){
  return `<div class="content"><div class="hero"><div class="card hero-main"><div class="muted small">${esc(state.competition || 'Nessuna competizione selezionata')}</div><h1>Top</h1><p>Una sola pagina, più pulita e più forte visivamente: tre tabelle affiancate per <strong>Marcatori</strong>, <strong>Assist</strong> e <strong>Valutazioni</strong>, con valori ben visibili, squadra e presenze in una sola vista più ordinata.</p><div class="top-overview"><div class="top-table green"><div class="top-table-head"><em>Verde</em><h2 style="margin-bottom:6px">Marcatori</h2><div class="muted small">Gol realizzati nella competizione selezionata.</div></div>${rankingView('scorers')}</div><div class="top-table white"><div class="top-table-head"><em>Bianco</em><h2 style="margin-bottom:6px">Assist</h2><div class="muted small">Passaggi decisivi e rifinitura migliore.</div></div>${rankingView('assists')}</div><div class="top-table red"><div class="top-table-head"><em>Rosso</em><h2 style="margin-bottom:6px">Valutazioni</h2><div class="muted small">Media voto più alta disponibile nei dati.</div></div>${rankingView('ratings')}</div></div></div><div class="card hero-side"><h2>Contesto</h2><div class="kpi"><div class="row"><strong>Competizione</strong><div class="muted small">${esc(state.competition || 'Da selezionare')}</div></div><div class="row"><strong>Vista</strong><div class="muted small">Top unificata più ampia</div></div><div class="row"><strong>Dettagli</strong><div class="muted small">Squadra, presenze e valori più leggibili</div></div></div></div></div></div>`;
}
...
function setPage(page){
  if(page==='home'){ resetToHome(); render(); return; }
...
function selectCompetition(name){
  ...
  if(item?.mode==='team' && item?.teamId){
    state.page='teams';
    openNationalTeam(item.teamId, item.name);
    return;
  }
  state.page='competition';
  loadCompetitionData();
